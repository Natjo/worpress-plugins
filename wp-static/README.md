...
À l'activation/désactivation, une tâche planifiée quotidienne est créée/supprimée (voir [Tâche planifiée](#tâche-planifiée-cron)).
...
- **Manuel** : **aucune** page n'est régénérée automatiquement (ni le cron). Toute modification marque le site « à régénérer » et affiche la **notice rouge** + l'indicateur rouge dans la barre d'admin. La mise à jour se fait via le bouton **Générer** ou les boutons **Régénérer** par ligne.
...
- **Complet** : l'**intégralité du site** est régénérée à **chaque** événement (sauvegarde, suppression, commentaire, terme…). Simple et toujours à jour, idéal pour les **petits sites** au contenu peu changeant — à éviter sur les gros sites car le coût croît avec le nombre de pages.
...
Dans l'onglet **Paramètres**, une case **« Minification »** : si activée, le HTML des pages statiques générées est compressé (espaces entre balises, espaces multiples et commentaires HTML retirés). Le contenu des balises `<pre>`, `<textarea>`, `<script>`, `<style>` et les commentaires conditionnels IE sont préservés.
...
- **Régénération planifiée** : choix entre désactivée, deux fois par jour, quotidienne ou hebdomadaire.
...
- Les contenus dont le **type a été retiré du menu d'administration** (ex. « Articles » masqué via `remove_menu_page('edit.php')`) ne sont **pas générés** : considérés comme inutilisés, comme dans le tableau. La liste des types masqués est mise en cache (option `wp_static_hidden_post_types`) à chaque affichage de l'admin, afin de rester fiable lors de la génération (`admin-post.php`) et du cron, où le menu n'est pas construit.
...
- En environnement local Docker, si l'appel au domaine public échoue, le plugin réessaie automatiquement via le **service web interne** (par défaut `nginx`) en conservant l'en-tête `Host` public. Voir [Constantes](#constantes-filtres-et-hooks).
...
- jamais pour un **utilisateur connecté** (cookies `wordpress_logged_in`, `comment_author`, `wp-postpass`) ;
- jamais pendant une **génération** (en-tête jeton `X-WP-Static-Token`) ;
...
À l'enregistrement d'un contenu (`save_post`), le plugin régénère, **en fin de requête** (pour ne pas ralentir l'éditeur) :
...
Certains changements ont un impact **global** non devinable : le plugin pose alors un drapeau et affiche une notice d'administration invitant à régénérer le site. Déclencheurs :
...
La notice apparaît **en rouge**. Un indicateur **WP Statique** est aussi présent dans la **barre d'administration** (header) : vert = service actif, gris = service désactivé, **rouge = régénération nécessaire**.
...
| `WP_STATIC_INTERNAL_HOST` | Hôte interne pour les requêtes de génération (Docker) | `nginx` si `ENV_LOCAL` |

```php
// Exemple : forcer l'hôte interne utilisé pour la génération
define( 'WP_STATIC_INTERNAL_HOST', 'mon-service-nginx' );
```
...
| Filtre | Description |
|---|---|
| `wp_static_internal_host` | Surcharge l'hôte interne de génération |
...
- Le dossier est entièrement nettoyé à la désactivation du plugin.
